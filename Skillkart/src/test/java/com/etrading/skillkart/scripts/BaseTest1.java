...
public class BaseTest1  {
	public class BaseTest implements AutoConstants
	{
							...
							@Optional(ETO)String explicit)
		{
			...
			appUrl=PropertyFile.getpropertyValue("./resources/config.properties", "appUrl");
			driver.get(appUrl);
		...
		public void loginToApp(@Optional(DEFAULT_USERNAME)String un,
							   @Optional(DEFAULT_PASSWORD)String pwd) throws Exception
		{
			LoginPage lp = new LoginPage(driver, webActionUtil);
			un=PropertyFile.getpropertyValue("./resources/config.properties", "username");
			pwd=PropertyFile.getpropertyValue("./resources/config.properties", "password");
			lp.login(un, pwd);
		}
